    ...
    if (empID === "") {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                if (dataFound == 0) {
                    $(".yearFilter, .particularFilter, #btnUndertime").addClass(
                        "d-none"
                    );
                    $("#vlBal, #slBal, #totalBalance").val(0);
                } else {
                    $(
                        ".yearFilter, .particularFilter, #btnUndertime"
                    ).removeClass("d-none");
...
hoursLate.addEventListener("keyup", function () {
    ...
    equivalent.value = (
        parseFloat(hoursLate.value) * 0.125 +
        parseFloat(minsLate.value) * 0.002 +
        parseFloat(hoursUndertime.value) * 0.125 +
        parseFloat(minsUndertime.value) * 0.002
    ).toFixed(3);
});
        ...
        if (willDelete) {
            $.ajax({
                ...
                success: function (response) {
                    swal(
                        "Good job!",
                        "Successfully deleted undertime record.",
                        "success",
                        { closeOnClickOutside: false }
                    ).then((isClicked) => {
                        if (isClicked) {
