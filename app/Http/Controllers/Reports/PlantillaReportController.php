...
class PlantillaReportController extends Controller
{
    ...
    private function getType(string $status)
    {
        $type = 'P';

        if (strtoupper($status) === 'PERMANENT') {
            $type = 'P';
        } else if (strtoupper($status) === 'ELECTED') {
            $type = 'E';
        } else if (strtoupper($status) === 'COTERMINOUS') {
            $type = 'C';
        }
        ...
        foreach ($records as $index => $plantillaPositions) {
            ...
            foreach ($plantillaPositions as $data) {
                if ($data->plantilla_history->isEmpty()) {
                    ...
                    list($current, $previous) = array_values($grades);
