...
class DBMPlantillaReportController extends Controller
{
    ...
    public function generate(string $office, string $year)
    {
        ...
        $positions = PlantillaPosition::with(['plantillas', 'position', 'plantillas.Employee', 'plantillas.division',  'plantilla_history', 'plantilla_history.Employee', 'salary_grade' => function ($query) use ($year, $previousYear) {
            $query->where('sg_year', $year)->orWhere('sg_year', $previousYear);
        }])->where('office_code', $office)->get()->groupBy(function ($record) {
        ...
        foreach($positions as $officeAndDivision => $p) {
            list($office, $division) = explode(' - ', $officeAndDivision);
            ...
            foreach($p as $position) {
                ...
                if($division) {
                    ...
                    $worksheet->mergecells($firstCell . $secondCell)->getCell("A" . $range - 1)->setValue('   ' . $division);
                    $worksheet->getStyle('A' . $range - 1)->getAlignment()->setHorizontal(Alignment::HORIZONTAL_LEFT);
                    $worksheet->getStyle('A' . $range - 1)->getFont()->setBold(true);
                    $range = $range + 1;
