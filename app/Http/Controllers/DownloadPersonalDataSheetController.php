...
class DownloadPersonalDataSheetController extends Controller implements IDownloadType
{
    ...
    {
        $hasSeperateWorkExperience = false;
        ...
        if ($workExperiences->count() >= 29) {
            $hasSeperateWorkExperience = true;
        }


        $spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load(public_path() . '\\PDS-TEMPLATE.xlsx');
        if ($hasSeperateWorkExperience) {
            $workExperienceSpreadSheet = \PhpOffice\PhpSpreadsheet\IOFactory::load(public_path() . '\\WORK_EXPERIENCE.xlsx');
        }
        ...
        $iterations = 1;
        $newIndex = 6;
        $workExperiences->each(function ($row) use (&$worksheet, &$index, &$iterations, &$workExperienceSpreadSheet, &$newIndex) {
            if ($iterations >= 29) {
                $sheet = $workExperienceSpreadSheet->getActiveSheet();

                $sheet->setCellValue("A" . $newIndex, Carbon::parse($row->from)->format('m/d/Y'));
                $sheet->setCellValue("C" . $newIndex, Carbon::parse($row->to)->format('m/d/Y'));
                $sheet->setCellValue("D" . $newIndex, Str::upper($row->position_title));
                $sheet->setCellValue("G" . $newIndex, Str::upper($row->office));
                $sheet->setCellValue("J" . $newIndex, $row->monthly_salary);
                $sheet->setCellValue("K" . $newIndex, ' ' . $row->salary_job_pay_grade);
                $sheet->setCellValue("L" . $newIndex, Str::upper($row->status_of_appointment));
                $sheet->setCellValue("M" . $newIndex, Str::upper($row->government_service));
        ...
        if ($hasSeperateWorkExperience) {
            $workExperienceWriter = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($workExperienceSpreadSheet, 'Xls');
            $workExperienceWriter->save(storage_path() . '\\generated_pds\\' . Str::upper(trim($employee->Employee_id)) . '-WORK_EXPERIENCE.xls');
        }

        return response()->json(['filename' => $fileName, 'success' => true, 'seperate_work_experience' => $hasSeperateWorkExperience]);
    }
