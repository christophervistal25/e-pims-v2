...
export default {
  ...
  methods: {
    ...
    openTab(selectedTab) {
      ...
      localStorage.setItem("create__current__tab", tabIndex);
    },
    openNextTab() {
      ...
      this.tabs.map((tab, index) => {
        if (currentTabName == tab.name) {
          ...
          localStorage.setItem("create__current__tab", index + 1);
        } else {
          tab.status = false;
        }
      });
  ...
  created() {
    if (localStorage.getItem("create__current__tab")) {
      this.selectedTab =
        this.tabs[localStorage.getItem("create__current__tab")];
    } else {
